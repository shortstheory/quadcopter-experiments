...
int32_t Adafruit_BMP085::readPressure(uint16_t temp) {
...
int32_t Adafruit_BMP085::readSealevelPressure(float altitude_meters) {
  float pressure = readPressure(readRawTemperature());
...
float Adafruit_BMP085::readAltitude(float sealevelPressure, uint16_t temp) {
  float altitude;
